...
import (
	...
	"KubeGale/model/cloudCmdb/response"
	"regexp" // For sqlmock
	"testing"
	"time"
...
// - Filtering by ProviderIDs
// - Filtering by both ProviderIDs and ResourceTypes
// - Case where CloudPlatform query returns an error
// - Case where one of the count queries returns an error (service should log and continue)
// - Case with no CloudPlatforms found
package cloudCmdb
...
import (
	...
	"regexp" // For sqlmock
	"testing"
	// "time" // Not used in this snippet, can be removed if not needed for other tests

...
func (m MetricsService) Health(clusterId int) (url string, httpClient http.Client, err error) {
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("prometheus service ready request failed", zap.Any("err", err))
		return
	}
	...
	if readyResp.StatusCode != http.StatusOK {
		global.KUBEGALE_LOG.Error("prometheus resp no ready", zap.Any("err", err))
		return
	}
	...
	for i := 0; i < e.NumField(); i++ {
		...
		go func(i int) {
			...
			if prometheusQueries == nil {
				global.KUBEGALE_LOG.Error("prometheusQueries nill failed", zap.Any("err", err))
				return
			}
			...
			if err != nil {
				global.KUBEGALE_LOG.Error("Prometheus health failed", zap.Any("err", err))
				return
			}

			//http Get请求 Prometheus接口
			promql := url.QueryEscape(prometheusQueries.GetValueByField(fName))
			global.KUBEGALE_LOG.Info(fmt.Sprintf("promql: %s ", prometheusQueries.GetValueByField(fName)))
			...
			if err != nil {
				global.KUBEGALE_LOG.Error("request metrics data failed", zap.Any("err", err))
				return
			}
			...
			if err != nil {
				global.KUBEGALE_LOG.Error("read response body failed", zap.Any("err", err))
				return
			}

			var data metrics.PrometheusQueryResp
			if err := json.Unmarshal(body, &data); err != nil {
				global.KUBEGALE_LOG.Error("unmarshal response body to models.PrometheusQueryResp failed", zap.Any("err", err))
				return
			}

...
import (
	...
	cluster3 "KubeGale/model/kubernetes/cluster"
	cluster2 "KubeGale/service/kubernetes/cluster"
...
func (k *K8sCloudTTYService) Get(tty cloudTTY.CloudTTY, uuid uuid.UUID) (podMsg *PodMessage, err error) {
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("get cluster info failed: " + err.Error())
		return podMsg, err
	}
	// 获取集群信息
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("get ConfigMap Message failed: " + err.Error())
		return podMsg, err
	}
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("get cloudshell Message failed: " + err.Error())
		return podMsg, err
	}
	// 生成transport
	ts, err := kubernetes.GenerateTLSTransport(&cluster, &user, true)
	if err != nil {
		global.KUBEGALE_LOG.Error("transport generate failed: " + err.Error())
		return podMsg, err
	}
	...
	if err = k.DeleteConfigMap(cluster, kubeconfigName, httpClient); err != nil {
		global.KUBEGALE_LOG.Error("Delete ConfigMap failed: " + err.Error())
		return podMsg, err
	}

	// 创建ConfigMap
	if err = k.CreateConfigMap(cluster, configmap, httpClient); err != nil {
		global.KUBEGALE_LOG.Error("Create ConfigMap failed: " + err.Error())
		return podMsg, err
	}

	// 创建 CreateCloudshell
	if err = k.CreateCloudshell(cluster, cloudshell, httpClient); err != nil {
		global.KUBEGALE_LOG.Error("Create Cloudshell failed: " + err.Error())
		return podMsg, err
	}
...
func (t *K8sCloudTTYService) CreateCloudshell(cluster cluster3.K8sCluster, body []byte, httpClient *http.Client) (err error) {
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("url Parse failed: " + err.Error())
		return err
	}

	req, err := http.NewRequest("POST", apiUrl.String(), bytes.NewBuffer(body))
	if err != nil {
		global.KUBEGALE_LOG.Error("http request failed: " + err.Error())
		return err
	}
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("http request do failed: " + err.Error())
		return err
	}
...
func (t *K8sCloudTTYService) getConfigMap(user cluster3.User, cluster cluster3.K8sCluster) (configmap []byte, kubeconfigName string, err error) {
	...
	if err = json.Unmarshal([]byte(user.KubeConfig), &jsonObj); err != nil {
		global.KUBEGALE_LOG.Error("Error unmarshalling JSON: " + err.Error())
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("Error marshalling to YAML: " + err.Error())
		return configmap, kubeconfigName, err
	}
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("Error JSON Marshal: " + err.Error())
		return configmap, kubeconfigName, err
	}

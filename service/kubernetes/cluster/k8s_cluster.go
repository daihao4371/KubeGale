...
import (
	...
	kubernetes2 "KubeGale/model/kubernetes"
...
func (k8sClusterService *K8sClusterService) CreateK8sCluster(k8sCluster *cluster2.K8sCluster) (err error) {
	if !errors.Is(global.KUBEGALE_DB.Where("name = ?", k8sCluster.Name).First(&cluster2.K8sCluster{}).Error, gorm.ErrRecordNotFound) {
	...
	if err := client.CheckPermissions(); err != nil {
		global.KUBEGALE_LOG.Error("perrmission failed: " + err.Error())
	}
	k8sCluster.UUID = uuid.Must(uuid.NewV4())
	if err := global.KUBEGALE_DB.Create(&k8sCluster).Error; err != nil {
		return err
	}
	...
	go func(k8sCluster *cluster2.K8sCluster) {
		...
		if err != nil {
			global.KUBEGALE_LOG.Error("get Ca.crt failed: " + err.Error())
		}

		if err = global.KUBEGALE_DB.Model(&cluster2.K8sCluster{}).Where("id = ?", k8sCluster.ID).Update("kube_ca_crt", caCrt).Error; err != nil {
			global.KUBEGALE_LOG.Error("update cluster filed: " + err.Error())
		}
	}(k8sCluster)
	// 异步创建集群默认角色
	go func() {
		if err := client.CreateDefaultClusterRoles(); err != nil {
			global.KUBEGALE_LOG.Error("create default cluster roles filed: " + err.Error())
		}
	}()
...
func (k8sClusterService *K8sClusterService) DeleteK8sCluster(ID int) (err error) {
	...
	if err = global.KUBEGALE_DB.Where("id = ?", ID).First(&cl).Error; errors.Is(err, gorm.ErrRecordNotFound) {
		return err
	}

	client := kubernetes.NewKubernetes(&cl, &cluster2.User{}, true)
	_ = client.CleanAllRBACResource()
	if err = global.KUBEGALE_DB.Delete(&cl).Error; err != nil {
		return err
	}
	return err
}
...
func (k8sClusterService *K8sClusterService) DeleteK8sClusterByIds(ids request.IdsReq) (err error) {
	for _, id := range ids.Ids {
		go func(id int) {
			var cl cluster2.K8sCluster
			if err = global.KUBEGALE_DB.Where("id = ?", id).First(&cl).Error; errors.Is(err, gorm.ErrRecordNotFound) {
				global.KUBEGALE_LOG.Error("search  cluster filed: " + err.Error())
			}
			...
			if err = global.KUBEGALE_DB.Delete(&cl).Error; err != nil {
				global.KUBEGALE_LOG.Error("delete cluster filed: " + err.Error())
			}
		}(id)
	}
	return
...
func (k8sClusterService *K8sClusterService) UpdateK8sCluster(k8sCluster cluster2.K8sCluster) (err error) {
	err = global.KUBEGALE_DB.Model(&cluster2.K8sCluster{}).Where("id = ?", k8sCluster.ID).Updates(&k8sCluster).Error
	return err
}
...
func (k8sClusterService *K8sClusterService) GetK8sCluster(id int) (k8sCluster cluster2.K8sCluster, err error) {
	err = global.KUBEGALE_DB.Where("id = ?", id).Preload("Users").First(&k8sCluster).Error
	return
}
func (k8sClusterService *K8sClusterService) GetK8sClusterByName(name string) (k8sCluster cluster2.K8sCluster, err error) {
	err = global.KUBEGALE_DB.Where("name = ?", name).First(&k8sCluster).Error
	return
}
...
func (K8sClusterService *K8sClusterService) CreateCredential(clusterId int, userId uint) error {
	...
	if err := global.KUBEGALE_DB.Where("id = ?", clusterId).Preload("Users", "uuid = ?", user.UUID).First(&clusterIns).Error; err != nil {
		return err
	}
	if !errors.Is(global.KUBEGALE_DB.Where("cluster_id = ? and uuid = ?", clusterId, user.UUID).First(&cluster2.User{}).Error, gorm.ErrRecordNotFound) {
	...
	// save User
	if err = global.KUBEGALE_DB.Save(&cluster2.User{
		UUID:       user.UUID,
...
func (k8sClusterService *K8sClusterService) GetClusterUserById(clusterId int, userId uint) (user []cluster2.User, err error) {
	...
	if err := global.KUBEGALE_DB.Where("id = ?", userId).First(&userObj).Error; err != nil {
		return user, err
	}

	var cluster cluster2.K8sCluster
	if err := global.KUBEGALE_DB.Where("id = ?", clusterId).Preload("Users", "uuid = ?", userObj.UUID).First(&cluster).Error; err != nil {
		return user, err
	}
...
func (k8sClusterService *K8sClusterService) GetClusterByUserUUID(id int, uuid uuid.UUID) (user cluster2.User, err error) {
	err = global.KUBEGALE_DB.Where("cluster_id = ? and uuid = ?", id, uuid).First(&user).Error
	return
}
...
func (k8sClusterService *K8sClusterService) GetClusterRoles(roleType clusterReq.ClusterRoleType) (roles []rbacV1.ClusterRole, err error) {
	var clusterIns cluster2.K8sCluster
	if err := global.KUBEGALE_DB.Where("id = ?", roleType.ClusterId).First(&clusterIns).Error; err != nil {
		return roles, err
	}
...
func (k8sClusterService *K8sClusterService) GetClusterApiGroups(apiGroups clusterReq.ClusterApiGroups) (groups []kubernetes2.ApiGroupOption, err error) {
	var clusterIns cluster2.K8sCluster
	if err := global.KUBEGALE_DB.Where("id = ?", apiGroups.ClusterId).First(&clusterIns).Error; err != nil {
		return nil, err
	}
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("ServerGroupsAndResources get resources failed:" + err.Error())
		return nil, err
	}
...
func (k8sClusterService *K8sClusterService) CreateClusterRole(role cluster2.RoleData) (err error) {
	...
	var clusterIns cluster2.K8sCluster
	if err := global.KUBEGALE_DB.Where("id = ?", role.ClusterId).First(&clusterIns).Error; err != nil {
		return err
	}
...
func (k8sClusterService *K8sClusterService) CreateUser(role clusterReq.CreateClusterRole) (err error) {
	...
	for _, userUuid := range role.UserUuids {
		...
		go func(u string, r clusterReq.CreateClusterRole) {
			...
			if err = global.KUBEGALE_DB.Where("uuid = ?", u).First(&user).Error; err != nil {
				global.KUBEGALE_LOG.Warn("User Not Found failed:" + err.Error())
				return
			}
			// 检查该用户是否已经存在于指定的集群中
			if !errors.Is(global.KUBEGALE_DB.Where("cluster_id = ? and uuid = ?", r.ClusterId, user.UUID).First(&cluster2.User{}).Error, gorm.ErrRecordNotFound) {
				global.KUBEGALE_LOG.Warn(fmt.Sprintf("The User: %s cluster credentials for this account already exist", user.NickName))
				return
			}
			// 调用 CreateClusterUser 方法来为用户创建集群凭证
			if err = k8sClusterService.CreateClusterUser(r, user); err != nil {
				global.KUBEGALE_LOG.Error("CreateClusterUser failed: " + err.Error())
				return
			}
...
func (k8sClusterService *K8sClusterService) CreateClusterUser(role clusterReq.CreateClusterRole, user system.SysUser) (err error) {
	...
	if err = global.KUBEGALE_DB.Where("cluster_id = ? and uuid = ?", role.ClusterId, user.UUID).First(&userData).Error; err != nil {
		return err
	}
		...
		go func(id uint, clusterRolesString string) {
			...
			for r := range role.ClusterRoles {
				if err = k.CreateOrUpdateClusterRoleBinding(role.ClusterRoles[r], user.Username, false); err != nil {
					global.KUBEGALE_LOG.Error("CreateOrUpdateClusterRoleBinding failed:" + err.Error())
				}
			}

			// 更新集群授权
			if err = global.KUBEGALE_DB.Model(&cluster2.User{}).Where("id = ?", id).Update("cluster_roles", clusterRolesString).Error; err != nil {
				global.KUBEGALE_LOG.Error("CreateOrUpdateClusterRoleBinding Cluster Role failed:" + err.Error())
			}
	...
	if len(role.NamespaceRoles) > 0 {
		...
		go func(id uint, namespaceRolesString string) {
			...
			for r := range role.NamespaceRoles {
				for j := range role.NamespaceRoles[r].Roles {
					if err := k.CreateOrUpdateRolebinding(role.NamespaceRoles[r].Namespace, role.NamespaceRoles[r].Roles[j], user.Username, false); err != nil {
						global.KUBEGALE_LOG.Error("CreateOrUpdateRolebinding Namespace role failed:" + err.Error())
					}
			...
			// 更新命名空间授权
			if err = global.KUBEGALE_DB.Model(&cluster2.User{}).Where("id = ?", id).Update("cluster_roles", namespaceRolesString).Error; err != nil {
				global.KUBEGALE_LOG.Error("CreateOrUpdateClusterRoleBinding Cluster Role failed:" + err.Error())
			}
...
func (k8sClusterService *K8sClusterService) UpdateClusterUser(role clusterReq.CreateClusterRole) (err error) {
	var user system.SysUser
	if err := global.KUBEGALE_DB.Where("uuid = ?", role.UUID).First(&user).Error; err != nil {
		return err
	}

	var userData cluster2.User
	if errors.Is(global.KUBEGALE_DB.Where("cluster_id = ? and uuid = ?", role.ClusterId, user.UUID).First(&userData).Error, gorm.ErrRecordNotFound) {
		return errors.New("该账号集群凭据不存在")
	}

	var clusterIns cluster2.K8sCluster
	if err := global.KUBEGALE_DB.Where("id = ?", role.ClusterId).Preload("Users", "uuid = ?", role.UUID).First(&clusterIns).Error; err != nil {
		return err
	}
	...
	if err := k.CleanManagedClusterRoleBinding(user.Username); err != nil {
		global.KUBEGALE_LOG.Error("CleanManagedClusterRoleBinding failed:" + err.Error())
	}

	// 清理Rolebinding
	if err := k.CleanManagedRoleBinding(user.Username); err != nil {
		global.KUBEGALE_LOG.Error("CleanManagedRoleBinding failed:" + err.Error())
	}
	...
	if len(role.ClusterRoles) > 0 {
		...
		// 集群授权清空
		if err = global.KUBEGALE_DB.Model(&cluster2.User{}).Where("id = ?", userData.ID).Update("cluster_roles", "").Error; err != nil {
			return err
		}
	...
	if len(role.NamespaceRoles) > 0 {
		...
		go func(id uint, namespaceRolesString string) {
			...
			for r := range role.NamespaceRoles {
				for j := range role.NamespaceRoles[r].Roles {
					if err := k.CreateOrUpdateRolebinding(role.NamespaceRoles[r].Namespace, role.NamespaceRoles[r].Roles[j], user.Username, false); err != nil {
						global.KUBEGALE_LOG.Error("CreateOrUpdateRolebinding failed:" + err.Error())
					}
				}
			}

			// 更新命名空间授权
			if err = global.KUBEGALE_DB.Model(&cluster2.User{}).Where("id = ?", id).Update("namespace_roles", namespaceRolesString).Error; err != nil {
				global.KUBEGALE_LOG.Error("CreateOrUpdateRolebinding  Namespace Role failed:" + err.Error())
			}
		...
		// 命名空间授权清空
		if err = global.KUBEGALE_DB.Model(&cluster2.User{}).Where("id = ?", userData.ID).Update("namespace_roles", "").Error; err != nil {
			return err
		}
	}
...
func (k8sClusterService *K8sClusterService) DeleteClusterUser(role clusterReq.DeleteClusterRole) (err error) {
	for _, userUuid := range role.UserUuids {
		...
		if err := global.KUBEGALE_DB.Where("cluster_id = ? and uuid = ?", role.ClusterId, userUuid).First(&user).Error; err != nil {
			global.KUBEGALE_LOG.Warn("Cluster Not Found User")
			continue
		}
		if err := global.KUBEGALE_DB.Transaction(func(tx *gorm.DB) error {
			var clusterIns cluster2.K8sCluster
			if err := global.KUBEGALE_DB.Where("id = ?", role.ClusterId).First(&clusterIns).Error; err != nil {
				global.KUBEGALE_LOG.Warn("Cluster Search failed: " + err.Error())
				return err
			}
			// 清理集群角色绑定
			k := kubernetes.NewKubernetes(&clusterIns, &cluster2.User{}, true)
...
func (k8sClusterService *K8sClusterService) GetClusterUserNamespace(clusterId int, uuid uuid.UUID) (nsList []string, err error) {
	var user cluster2.User
	if err := global.KUBEGALE_DB.Where("cluster_id = ? and uuid = ?", clusterId, uuid).First(&user).Error; err != nil {
		return nil, err
	}

	var cluster cluster2.K8sCluster
	if err := global.KUBEGALE_DB.Where("id = ?", clusterId).First(&cluster).Error; err != nil {
		return nil, err
	}
...
func (k8sClusterService *K8sClusterService) GetClusterListNamespace(clusterId int) (nsList []corev1.Namespace, err error) {
	var clusterIns cluster2.K8sCluster
	if err := global.KUBEGALE_DB.Where("id = ?", clusterId).First(&clusterIns).Error; err != nil {
		return nil, err
	}
...
func (k8sClusterService *K8sClusterService) GetK8sClusterInfoList(info clusterReq.K8sClusterSearch) (list []cluster2.K8sCluster, total int64, err error) {
	...
	// 创建db
	db := global.KUBEGALE_DB.Model(&cluster2.K8sCluster{})

<template>
  <div class="nodetty-container">
    ...
    <div class="terminal-content">
      <div class="terminal-window" ref="terminalRef">
        <div v-if="!isConnected" class="connection-prompt">
          <el-empty description="点击连接按钮开始节点终端会话">
            <el-button type="primary" @click="handleConnect">连接节点终端</el-button>
          </el-empty>
        </div>
        <div v-else class="terminal-output">
          ...
          <div class="terminal-input-line">
            <span class="prompt">root@{{ nodeName }}:~# </span>
            <input 
              ref="inputRef"
    ...
    <div class="terminal-footer">
      <span class="status">{{ isConnected ? '已连接到节点' : '未连接' }}</span>
...
const handleConnect = () => {
  ...
  terminalOutput.value.push('连接到节点终端...')
  terminalOutput.value.push(`欢迎使用节点 ${props.nodeName} 终端`)
...
const handleDisconnect = () => {
  ...
  terminalOutput.value.push('节点终端连接已断开')
}
...
const handleCommand = () => {
  ...
  terminalOutput.value.push(`root@${props.nodeName}:~# ${command}`)

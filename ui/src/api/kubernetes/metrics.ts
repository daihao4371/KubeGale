...
export const getLastMetricValues = (metrics: any, nodeName: any, metricNames: any) => {
  return metricNames.map((metricName: any) => {
    try {
      const metric = metrics[metricName]
      const result = metric.data.result.find((result: any) => {
        return [
          result.metric.node,
...
export function getMetricLastPoints(metrics: any) {
...
export const normalizeMetrics = (metrics: any, frames = 60) => {
  if (
    metrics === null ||
    metrics.data === null ||
      ...
      result.forEach((res: any) => {
        ...
        while (timestamp <= now) {
          timestamp = moment
            ...
            .add(1, 'minute')
            .unix()
          if (!res.values.find((value: any) => value[0] === timestamp)) {
...
export const NodeMetricsList = (params: any) => {
  return service({

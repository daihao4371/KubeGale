...
func GetToken(c *gin.Context) string {
	...
	if token == "" {
		...
		if err != nil {
			global.KUBEGALE_LOG.Error("重新写入cookie token失败,未能成功解析token,请检查请求头是否存在x-token且claims是否为规定结构")
			return token
		}
...
func LoginToken(user system.Login) (token string, claims systemReq.CustomClaims, err error) {
	j := &JWT{SigningKey: []byte(global.KUBEGALE_CONFIG.JWT.SigningKey)} // 唯一签名

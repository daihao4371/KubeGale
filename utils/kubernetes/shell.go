...
import (
	...
	"KubeGale/model/kubernetes/ws"
	request2 "KubeGale/model/system/request"
	"bytes"
	"encoding/json"
	"fmt"
...
func (k *KubeShell) WriteLog(terminal ws.TerminalRequest, user *request2.CustomClaims) (err error) {
	...
	if len(recorders) != 0 {
		...
		if err := global.KUBEGALE_DB.Where("id = ?", terminal.ClusterId).First(&cluster).Error; err != nil {
			global.KUBEGALE_LOG.Error("cluster get failed, err:", zap.Any("err", err))
			return err
		}
		...
		if err = global.KUBEGALE_DB.Save(&terminalAudit).Error; err != nil {
			global.KUBEGALE_LOG.Error("terminalAudit save failed, err:", zap.Any("err", err))

...
func (k *Kubernetes) CheckPermissions() (err error) {
	...
	if err != nil {
		global.KUBEGALE_LOG.Error(fmt.Sprintf("notAllowed faile: %s", err.Error()))
...
func (k *Kubernetes) HasPermission(attributes v1.ResourceAttributes) (kubernetes.PermissionCheckResult, error) {
	...
	if err != nil {
		global.KUBEGALE_LOG.Error("clientset init failed: " + err.Error())
...
func (k *Kubernetes) Client() (*k8s.Clientset, error) {
	cfg, err := k.Config()
	if err != nil {
		global.KUBEGALE_LOG.Error("Config init failed: ", zap.Error(err))
		return nil, err
	}

	clientset, err := k8s.NewForConfig(cfg)
	if err != nil {
		global.KUBEGALE_LOG.Error("clientset init failed: ", zap.Error(err))
		return nil, err
	}
...
func (k *Kubernetes) Config() (config *rest.Config, err error) {
	...
	if k.KubeType == 1 {
		if k.IsAdmin {
			...
			if err != nil {
				global.KUBEGALE_LOG.Error("RESTConfigFromKubeConfig init failed: ", zap.Error(err))
				return nil, err
			}
		} else {
...
func (k *Kubernetes) CreateClientCertificate(username string) (privateKey, publicCert string, err error) {
	...
	for {
		...
		for {
			...
			if err != nil {
				max_num += 1
				global.KUBEGALE_LOG.Warn("CertificateSigningRequests Get failed: " + err.Error())
				continue
			}
			// 检查CSR是否已经颁发证书
